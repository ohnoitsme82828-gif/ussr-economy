...
export async function initializeDatabase() {
  ...
  try {
    ...
    await client.query(`
      CREATE INDEX IF NOT EXISTS idx_rate_limits_key_created ON rate_limits(limit_key, created_at DESC);
    ...
    if (parseInt(adminCheck.rows[0].count) === 0) {
      ...
      if (!adminPassword) {
        console.warn('INITIAL_ADMIN_PASSWORD environment variable is not set. Skipping admin user creation. You can create an admin later via admin API.');
      } else {
